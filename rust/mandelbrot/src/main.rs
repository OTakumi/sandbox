...
/// Specifically, 's' shoukd have the form <left><sep><right>,
/// where <sep> is the character given by the 'separator' argument,
/// and <left> and <right> are both strings that can be parsed by T::from_str 'separator' must be an ASCII character.
/// If 's' has the proper from, return 'Some<(x, y)>'.
...
/// return the corresponding point on the complez plane.
