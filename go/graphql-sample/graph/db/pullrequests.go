...
func (o *Pullrequest) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(pullrequestPrimaryKeyColumns) == 0 {
				return errors.New("db: unable to upsert pullrequests, could not build conflict column list")
			}

			conflict = make([]string, len(pullrequestPrimaryKeyColumns))
			copy(conflict, pullrequestPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"pullrequests\"", updateOnConflict, ret, update, conflict, insert, opts...)
