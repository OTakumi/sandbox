...
func (o *Projectcard) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(projectcardPrimaryKeyColumns) == 0 {
				return errors.New("db: unable to upsert projectcards, could not build conflict column list")
			}

			conflict = make([]string, len(projectcardPrimaryKeyColumns))
			copy(conflict, projectcardPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"projectcards\"", updateOnConflict, ret, update, conflict, insert, opts...)
