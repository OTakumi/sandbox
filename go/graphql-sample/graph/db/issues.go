...
func (o *Issue) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(issuePrimaryKeyColumns) == 0 {
				return errors.New("db: unable to upsert issues, could not build conflict column list")
			}

			conflict = make([]string, len(issuePrimaryKeyColumns))
			copy(conflict, issuePrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"issues\"", updateOnConflict, ret, update, conflict, insert, opts...)
